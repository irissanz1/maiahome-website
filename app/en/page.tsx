...
const BENEFITS = [
  { t: "Prime locations", d: "Polanco, Condesa and Houston — the best areas to live or work." },
  { t: "Design-forward spaces", d: "Stylishly furnished apartments with WiFi, equipped kitchen and everything included." },
  { t: "Stays your way", d: "By the night, monthly or corporate housing." },
  { t: "Local expertise", d: "A team that knows every area and recommends the best of the city." },
];
...
export default async function HomeEn({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const featured = pinned.length >= 3 ? pinned.slice(0, 3) : [...pinned, ...marketProps.filter((p) => !FEATURED_SLUGS.includes(p.slug)).slice(0, 3 - pinned.length)];

  const mapMarkers: MapMarker[] = marketProps
    .filter((p) => typeof p.lat === "number" && typeof p.lng === "number")
    .map((p) => ({
      slug: p.slug, nombre: p.nombre, zonaNombre: p.zonaNombre, lat: p.lat as number, lng: p.lng as number,
      priceLabel: p.precioDesde != null ? `${formatMoney(p.precioDesde, p.currency)} / night` : null,
      image: img(p.images[0], 400), rating: p.rating, href: `/en/stay/${p.slug}`,
    }));
  const zoneMap = new Map<string, { slug: string; count: number }>();
  for (const p of marketProps) {
    const cur = zoneMap.get(p.zonaNombre);
    if (cur) cur.count += 1; else zoneMap.set(p.zonaNombre, { slug: p.zona, count: 1 });
  }
  const zones = [...zoneMap.entries()].sort((a, b) => b[1].count - a[1].count);

  return (
    <>
      <div className="mx-auto max-w-6xl px-5 pt-6"><HomeHero /></div>
      <div className="mt-6"><StatsBar lang="en" /></div>
      <div className="mt-10"><TrustBar lang="en" /></div>
      ...
      <section className="mx-auto max-w-6xl px-5 pb-4">
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {featured.map((p) => (<PropertyCard key={p.beds24RoomId} property={p} search={{}} lang="en" />))}
        </div>
      </section>
      ...
      {mapMarkers.length > 0 && (
        <section className="mx-auto max-w-6xl px-5 py-16">
          <div className="grid grid-cols-1 items-center gap-8 lg:grid-cols-[0.85fr_1.15fr]">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.28em] text-neutral-500">Where we are</p>
              <h2 className="mt-3 font-serif text-3xl text-neutral-900 md:text-4xl">In the heart of the best areas</h2>
              <p className="mt-3 text-neutral-600">
                Our apartments cluster in Polanco and Condesa (Mexico City) and Houston: steps from the best dining, culture and business life. Explore the map and choose by location.
              </p>
              <div className="mt-5 flex flex-wrap gap-2">
                {zones.map(([nombre, { slug, count }]) => (
                  <Link key={slug} href={`/en/${slug}`} className="inline-flex items-center gap-1.5 rounded-full border border-neutral-300 px-3.5 py-1.5 text-sm text-neutral-700 transition hover:border-maia-strong hover:bg-neutral-50">
                    {nombre}<span className="text-xs font-semibold text-neutral-400">{count}</span>
                  </Link>
                ))}
              </div>
              <Link href={`/en/apartments?market=${market.id}`} className="mt-6 inline-block rounded-full bg-maia-dark px-6 py-3 text-sm font-semibold text-white transition hover:bg-black">View all on the map →</Link>
            </div>
            <PropertiesMap markers={mapMarkers} heightClass="h-[420px] md:h-[460px]" showPois poiGroupKeys={["cultura", "compras", "parques"]} poiControl={false} />
          </div>
        </section>
      )}
      ...
      <section className="mt-4 bg-[#FBF7EC]">
        <div className="mx-auto flex max-w-6xl flex-col items-start gap-6 px-5 py-12 md:flex-row md:items-center md:justify-between">
          <div className="max-w-2xl">
            ...
            <h2 className="mt-3 font-serif text-2xl font-bold text-neutral-900 md:text-3xl">Staying with us also supports the community</h2>
            <p className="mt-2 text-neutral-600">
              Part of every stay goes to Fundación Altía, supporting children and communities in Mexico City. More than <strong>75 local initiatives</strong> supported.
            </p>
          </div>
          <Link href="/en/about" className="shrink-0 rounded-full bg-maia-dark px-6 py-3 text-sm font-semibold text-white transition hover:bg-black">Discover our purpose →</Link>
        </div>
      </section>
    </>
  );
