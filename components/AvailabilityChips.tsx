  ...
  return (
    <div className="mt-4 flex flex-wrap items-center gap-2">
      ...
      <Link href={chipUrl("si")} className={chipClass(disp === "si", "green")}>
        ...
        Disponibles <span className="opacity-80">· {available}</span>
      </Link>
      <Link href={chipUrl("no")} className={chipClass(disp === "no", "gray")}>
        <span className={`h-2 w-2 rounded-full ${disp === "no" ? "bg-white" : "bg-neutral-400"}`} />
        No disponibles <span className="opacity-80">· {unavailable}</span>
      </Link>
      {!hasDates && <span className="text-xs text-neutral-400">sin fechas: según calendario</span>}
    </div>
  );
}
